...
chrome.storage.sync.get((config) => {
    ...
    if (!config.orcLanguage) {
        chrome.storage.sync.set({ orcLanguage: 'eng' })
    }
        ...
        chrome.storage.sync.get((config) => {
            ...
            chrome.tabs.getSelected(null, (tab) => {
                ...
                chrome.tabs.captureVisibleTab(tab.windowId, { format: config.format }, (image) => {
                    crop(image, req.area, req.dpr, config.dpr, config.format, config.orcLanguage, tab,(cropped) => {
...
function crop(image, area, dpr, preserve, format, lang, tab, done) {
    var top = area.y * dpr
    var left = area.x * dpr
    ...
    img.onload = () => {
        ...
        Tesseract.recognize(cropped, {lang: lang})
            .progress(function(progress) {
                status = progress["status"] + " (" + (progress["progress"] * 100) + "%)"
                chrome.tabs.sendMessage(tab.id, { message: 'status', status: status })
            })
            .then(function (result) {
                chrome.tabs.sendMessage(tab.id, { message: 'loaded', result: result.text }, (res) => {
                    if (res) {
                        clearTimeout(timeout)
                    }
                });
